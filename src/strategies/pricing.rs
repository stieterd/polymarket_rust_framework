...
async fn bootstrap_historical_klines(shared_state: SharedState) -> Result<bool> {
    ...
    for _ in 0..(((RV_WINDOW_MINUTES + 2) * 60) / 1000 + 1) {
        ...
        let klines_batch: Vec<Vec<Value>> = client
            .get(BINANCE_SPOT_KLINES_URL)
            .query(&params)
            .send()
            .await?
    ...
    let unique_klines: HashMap<i64, &Vec<Value>> = all_klines
        .iter()
        .map(|k| (k[0].as_i64().unwrap(), k))
        .collect();
    ...
    while initial_atm_iv.is_none() {
        let result: Result<f64> = async {
            ...
            let var_expected_rv_window =
                calculate_expected_variance_in_window(t_start_rv, t_now, &variance_profiles);
            info!(
                "[IV DEBUG] var_expected_rv_window (for past {} min): {}",
                RV_WINDOW_MINUTES, var_expected_rv_window
            );
            ...
            let var_expected_future =
                calculate_expected_variance_in_window(t_now, t_expiry, &variance_profiles);
            info!(
                "[IV DEBUG] var_expected_future (until expiry): {}",
                var_expected_future
            );
...
async fn analysis_loop(
    ...
    loop {
        ...
        let result: Result<()> = async {
                ...
                let book = if let Some(orderbook_entry) = poly_state.orderbooks.get(&up_token_id) {
                    if let Ok(orderbook_lock) = orderbook_entry.read() {
                        ...
                        for bid in orderbook_lock.get_bid_map().iter() {
                            pricing_book.bids.insert(
                                (f64::from(*bid.key()) / 1000.0).to_string(),
                                (f64::from(*bid.value()) / 1000.0).to_string(),
                            );
                        }
                        for ask in orderbook_lock.get_ask_map().iter() {
                            pricing_book.asks.insert(
                                (f64::from(*ask.key()) / 1000.0).to_string(),
                                (f64::from(*ask.value()) / 1000.0).to_string(),
                            );
                        }
                        Some(pricing_book)
                    } else {
            ...
            let midpoint = (binance_tob.bid_price.parse::<Decimal>()?
                + binance_tob.ask_price.parse::<Decimal>()?)
                / dec!(2);
            let log_imbalance =
                (binance_tob.bid_qty.parse::<f64>()? / binance_tob.ask_qty.parse::<f64>()?).ln();
            let binance_fair_value = midpoint + predict_vwap_difference(log_imbalance);
            ...
            let market_prob = calculate_polymarket_book_vwap(&book, POLYMARKET_VWAP_LEVELS)
                .ok_or_else(|| anyhow!("Could not calculate PM VWAP"))?;
            let in_toxic_region =
                (TOXIC_REGION_LOWER_BOUND..TOXIC_REGION_UPPER_BOUND).contains(&market_prob);
            ...
            let status = if in_toxic_region {
                "TOXIC REGION: Mkt IV Frozen.".to_string()
            } else {
                "✅ Active".to_string()
            };
...
pub async fn pricing_main() {
    ...
    let market_config = autodiscover_market_config("bitcoin", "btc")
        .await
        .expect("Market discovery failed")
        .expect("Failed to find active market");
    ...
    let event_sender =
        spawn_event_processor(app_state.clone(), polymarket_state.clone(), strategies);
    info!("--- Event Processor thread has been started ---");

    let variance_profiles =
        Arc::new(load_variance_profile(PROFILE_FILENAME).expect("Failed to load variance profile"));
    let shared_state = Arc::new(tokio::sync::Mutex::new(SharedData {
        kline_closes: VecDeque::with_capacity(KLINE_WINDOW_SIZE),
        ..Default::default()
    }));
    ...
    let market_iv_window = initialize_smoothed_iv(
        shared_state.clone(),
        market_config.clone(),
        variance_profiles.clone(),
    )
    .await
    .expect("Failed to initialize smoothed IV window");
...
pub async fn autodiscover_market_config(
    auto_market: &str,
    crypto: &str,
) -> Result<Option<DiscoveredMarketConfig>> {
    ...
    let start_hour_et = now_et
        ...
        .ok_or_else(|| anyhow!("failed to normalize time"))?;
    let end_hour_et = start_hour_et + chrono::Duration::hours(1);
    let hour_str = start_hour_et
        .format("%I%p")
        .to_string()
    ...
    let market_slug = format!(
        "{}-up-or-down-{}-{}-{}-et",
        auto_market,
        start_hour_et.format("%B").to_string().to_lowercase(),
        day_str,
        hour_str
    );
    ...
    loop {
        let url = format!(
            "https://gamma-api.polymarket.com/events?limit=100&active=true&closed=false&offset={}",
            offset
        );
        let events: Value = client.get(&url).send().await?.json().await?;
        let events_arr = events
            .as_array()
            .ok_or_else(|| anyhow!("events not an array"))?;
        if events_arr.is_empty() {
            break;
        }
        if let Some(event) = events_arr.iter().find(|e| {
            e.get("slug")
                .and_then(Value::as_str)
                .map_or(false, |s| s.eq_ignore_ascii_case(&market_slug))
        }) {
    ...
    let target_event =
        target_event.ok_or_else(|| anyhow!("Market slug '{}' not found", market_slug))?;
    let markets = target_event
        .get("markets")
        .and_then(Value::as_array)
        .ok_or_else(|| anyhow!("event has no markets"))?;
    let (mut yes_token_id, mut no_token_id) = (None, None);

    for market in markets {
        if let (Some(Value::String(s_tokens)), Some(Value::String(s_outcomes))) =
            (market.get("clobTokenIds"), market.get("outcomes"))
        {
    ...
    let strike_price = kline_data
        .as_array()
        .and_then(|a| a.first())
        .and_then(|e| e.get(1))
        .and_then(Value::as_str)
        .and_then(|p| p.parse().ok())
        .ok_or_else(|| anyhow!("could not parse strike"))?;
            ...
            Ok(ws_stream) => {
                ...
                while let Some(msg_result) = read.next().await {
                    match msg_result {
                        ...
                        Ok(Message::Close(_)) => {
                            warn!("[{}] Connection closed by server.", listener);
                            break;
                        }
                        Err(e) => {
                            error!("[{}] WebSocket stream error: {}.", listener, e);
                            break;
                        }
                        _ => {}
                    }
...
impl Strategy for UpdateOrderbookStrategy {
    ...
    async fn poly_handle_market_agg_orderbook(
        ...
        ctx: &StrategyContext,
        _listener: Listener,
        snapshot: &AggOrderbook,
    ) {
        let orderbook = CoreOrderBook::new(snapshot);
        ctx.poly_state
            .orderbooks
            .insert(snapshot.asset_id.clone(), Arc::new(RwLock::new(orderbook)));
    }
...
impl EventProcessor {
    async fn handle_event(&self, event: SocketEvent) {
        if let SocketEvent::Market {
            listener,
            mut payload,
        } = event
        {
            self.handle_market_event(listener, &mut payload).await;
        }
    }
    ...
    async fn handle_market_event(&self, listener: Listener, payload: &mut [u8]) {
        if payload.is_empty() {
            return;
        }
        ...
        if let Ok(wrappers) = simd_json::from_slice::<Vec<PolymarketMessageWrapperOld>>(payload) {
            for w in wrappers {
                self.dispatch_market_message(listener, w).await;
            }
        } else if let Ok(w) = simd_json::from_slice::<PolymarketMessageWrapperOld>(payload) {
            self.dispatch_market_message(listener, w).await;
        } else {
            error!(
                "[{}] Failed to parse legacy market message. Raw: {}",
                listener,
    ...
    async fn dispatch_market_message(
        &self,
        listener: Listener,
